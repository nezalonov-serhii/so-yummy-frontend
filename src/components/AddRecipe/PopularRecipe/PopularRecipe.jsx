   ...
   return (
      <Container>
         ...
         {recipes.length < 1 ? (
            ...
            <List>
               {recipes
                  .slice(0, window.innerWidth >= 768 && window.innerWidth <= 1279 ? 2 : 4)
                  .map(({ _id, preview, title, description }) => {
                     const name = title.length < 23 ? title : title.slice(0, 18) + "...";
                     const paragraph =
                        description.length < 100 ? description : description.slice(0, 80) + "...";
                     const recipeId = _id;
                     return (
                        <Item key={_id}>
